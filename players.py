...
class Ship(object):
    def __init__(self, HP, x, y, speed_x, speed_y, sprites, sprites_death, damage):
        self.HP = HP
        self.x = 0
        self.y = 0
        self.set_coord(x, y)
    ...
    def set_coord(self, x, y):
        if (x < config.window_width - config.image_size and x > 0) and (y < config.window_high - config.image_size and y > 0):
            self.x = x
            self.y = y
        else:
            print('bad coords!')
...
class Bullet(Ship):
    def __init__(self, x, y, speed_x, sprites, sprites_death, damage, direction):
        HP = 0
        speed_y = 0
        Ship.__init__(self, HP, x, y, speed_x, speed_y, sprites, sprites_death, damage)
        self.direction = direction

    def set_coord(self, x, y):
        if (x < config.window_width - config.image_size and x > 0) and (y < config.window_high - config.image_size and y > config.image_size):
            self.x = x
            self.y = y
        else:
            self.on_damage(1)
...
class Hero(Ship):
    ...
    def fire(self, keys):
        if keys[pygame.K_SPACE] and self.anim_count % config.hero_frames_per_bullet == 0:
            return  Bullet(self.x + 6, self.y, config.bullet_speed, config.bullet_sprites, config.sprites_death, config.hero_bullet_damage, config.move_right)

#ребёнок класса Ship - враги
class Enemy(Ship):
    def __init__(self, HP = config.enemy_start_HP, x = 800, y = 0,
                    speed_x = config.enemy_speed, speed_y = config.enemy_speed, sprites = config.fast_ship_sprites, sprites_death = config.sprites_death,
                    damage = config.enemy_taran_damage, amplitude = config.enemy_trajectory_amplitude, phase = config.enemy_trajectory_phase, frequency = config.enemy_trajectory_frequency):
        if y == 0:
            y = random.randrange(config.enemy_random_y,config.window_high - config.enemy_random_y, config.enemy_delt_coord)
        if speed_x == 0:
            speed_x = 0 #random.random()*0.05
        if speed_y == 0:
            speed_y = 0 #random.random()*0.05
        Ship.__init__(self, HP, x, y, speed_x, speed_y, sprites, sprites_death, damage)
        if amplitude == 0:
        #    amplitude = random.random()*4 + 1
        ...
        self.amplitude = amplitude #min((self.y),(800 - self.y))
        self.start_x = x
        self.start_y = y
    ...
    def fire(self):
        if self.anim_count % config.enemy_frames_per_bullet == 0:
            return Bullet(self.x - config.enemy_bullet_indent, self.y, config.bullet_speed, config.bullet_sprites, config.sprites_death, config.enemy_taran_damage, config.move_left)
...
class Game(object):
    def __init__(self):
        self.player = Hero(config.hero_start_HP, config.hero_start_x, config.hero_start_y, config.hero_speed, config.hero_speed, config.player_sprites, config.sprites_death, config.hero_taran_damage)
